...
int WriteMatrixA(SprsMatRealStru *A, char *filename) {
    ...
    if (A->Mat.piIdiag != NULL) {
        ...
        return -5;
    memset(LineBuffer, 0, 1024);
    sprintf(LineBuffer, "SparsMatRealStruA->piLinkp,size=%d",
            A->Mat.iNymax + 1);
    fprintf(fp, "%s\n", LineBuffer);
    if (A->Mat.piLinkp != NULL) {
        ...
        return -6;
    memset(LineBuffer, 0, 1024);
    sprintf(LineBuffer, "SparsMatRealStruA->piLinkn,size=%d",
            A->Mat.iNymax + 1);
    fprintf(fp, "%s\n", LineBuffer);
...
int CompareVectorX(VecRealStru *X, int &nsize, VecRealStru *result) {
    ...
    for (i = 0; i < nsize; i++) {
        for (j = 0; j < X[i].iNy + 1; j++) {
            if ((X[i].pdVal[j] - result[i].pdVal[j] > 1e-9) ||
                ...
                fprintf(
                    fp,
                    "matrix dimensions=%dColumn number matrix =%d,reference "
    ...
    if (num > 0) {
        printf(" result exceed the set reference value  %d,please see the "
               "log.txt\n",
               num);
        return -1;
    }
